...
std::optional<NodeId> StepwisePathCalculator::step() {
    ...
    auto& data_to_fix = _node_data.at(next_id_to_fix);
    data_to_fix.dist_fixed = true;
    for (auto const& half_edge_id : _graph.node(next_id_to_fix).outgoing_halfedges()) {
        auto const& end_node = _graph.halfedge(half_edge_id).target();
        auto& end_data = _node_data.at(end_node);
        if (not end_data.dist_fixed) {
            auto const& edge_weight = _graph.edge_cost(half_edge_id, _cost_transform);
            ...
            if (end_data.distance > distance_via_node) {
                ...
                end_data.previous_half_edge = half_edge_id;
                heap::decrease_or_insert(_heap, end_node, distance_via_node);
            }
        }
    }
...
Path StepwisePathCalculator::get_path_to(NodeId const target) const {
    ...
    while (current_node != _source) {
        ...
        edges_reversed.push_back(_graph.edge_id(current_data.previous_half_edge));
        auto const& forward_halfedge = _graph.halfedge(current_data.previous_half_edge);
        current_node = _graph.halfedge(forward_halfedge.inverse()).target();
    }
